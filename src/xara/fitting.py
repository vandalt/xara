...
# =========================================================================
# =========================================================================
def vertical_rim(gsz=256, gstep=15, height=100, rad=450, cont=1e-3, inc=60, PA=310):
    ...
    xx, yy = np.meshgrid(
        gstep * (np.arange(gsz) - gsz / 2), gstep * (np.arange(gsz) - gsz / 2)
    )
...
def grid_src_KPD(mgrid, gscale, kpi, hdr, phi=None, deg=False):
    ...
    filter = hdr["filter"]
    cvis = core.grid_src_cvis(kpi.uv[:, 0], kpi.uv[:, 1], filter, mgrid, gscale, phi)
...
def grid_src_KPD_fit_residuals(mgrid, gscale, kpo, phi=None, deg=False, reg=0.0):
    ...
    err = kpo.kpd - test
    if kpo.kpe is not None:
        err /= kpo.kpe
    if reg != 0.0:
        err += reg * core.negentropy(mgrid + 1e-9)
    return err
...
def grid_src_KPD_fit(mgrid, gscale, kpo, phi=None, deg=False, reg=0.0):
    ...
    soluce = leastsq(
        grid_src_KPD_fit_residuals,
        mgrid,
        args=((gscale, kpo, phi, deg, reg)),
        full_output=1,
        ftol=1e-10,
        factor=10.0,
    )
...
def binary_model(params, kpi, hdr, vis2=False, deg=True):
    ...
    if "Hale" in hdr["tel"]:
        params2[1] += 220.0 + hdr["orient"]
    if "HST" in hdr["tel"]:
        params2[1] -= hdr["orient"]
    else:
        params2[1] += 0.0

    filter = hdr["filter"]

    testPhi = core.phase_binary(kpi.uv[:, 0], kpi.uv[:, 1], filter, params2, deg)
    res = np.dot(kpi.KerPhi, testPhi)

    if vis2:
        res = core.vis2_binary(kpi.uv[:, 0], kpi.uv[:, 1], filter, params2, deg)
    return res
...
def binary_KPD_fit(p0, kpo, kpdt, kpde=None):
    ...
    soluce = leastsq(
        binary_KPD_fit_residuals, p0, args=((kpo, kpdt, kpde)), full_output=1
    )  # , factor=0.1)
...
# =========================================================================
def chi2map_sep_con(
    kpo, pa=0, reduced=False, cmap=None, srng=[10.0, 200.0, 100], crng=[10.0, 50.0, 100]
):
    ...
    for i, sep in enumerate(seps):
        for j, con in enumerate(cons):
            chi2map[i, j] = np.sum(binary_KPD_fit_residuals([sep, pa, con], kpo) ** 2)
    ...
    asp = 1.0 / np.abs((srng[1] - srng[0]) / (crng[1] - crng[0]))
    plt.imshow(
        chi2map, aspect=asp, cmap=cmap, extent=[crng[0], crng[1], srng[0], srng[1]]
    )
...
def chi2map_sep_pa(
    ...
    for i, sep in enumerate(seps):
        for j, th in enumerate(angs):
            chi2map[i, j] = np.sum(binary_KPD_fit_residuals([sep, th, con], kpo) ** 2)
        sys.stdout.write("\r(%3d/%d): sep = %.2f mas" % (i + 1, srng[2], sep))
        sys.stdout.flush()
    ...
    asp = 1.0 / np.abs((srng[1] - srng[0]) / (arng[1] - arng[0]))

    plt.imshow(
        (chi2map), aspect=asp, cmap=cmap, extent=[arng[0], arng[1], srng[0], srng[1]]
    )
...
def super_cal_coeffs(src, cal, model=None, regul="None"):
    ...
    if model is not None:
        b -= model  # optional model subtraction

    if regul == "None":
        coeffs = np.dot(np.linalg.pinv(np.dot(A.T, A)), np.dot(A.T, b).T)
    else:
        coeffs = np.dot(
            np.linalg.pinv(np.dot(A.T, A) + np.identity(ns)), np.dot(A.T, b).T
        )
    return coeffs.T
...
def chi2_volume(src, cal=None):
    ...
    for i, sep in enumerate(seps):
        for j, ang in enumerate(angs):
            for k, con in enumerate(cons):
                res = core.binary_multi_KPD_fit_residuals([sep, ang, con], src)
...
# =========================================================================
# =========================================================================
def correlation_plot(kpo, params=[250.0, 0.0, 5.0], plot_error=True, fig=None):
    ...
    if plot_error:
        sp0.errorbar(
            core.binary_KPD_model(kpo, params), kpo.kpd, yerr=kpo.kpe, linestyle="None"
        )
    else:
        sp0.plot(core.binary_KPD_model(kpo, params), kpo.kpd, "bo")
    ...
    plt.text(0.0 * mm, -0.75 * mm, msg, bbox=dict(facecolor="white"), fontsize=14)
    ...
    plt.text(-0.75 * mm, 0.5 * mm, msg, bbox=dict(facecolor="white"), fontsize=14)
...
def nested(
    kpo,
    index=0,
    prange=[30.0, 250.0, 0.0, 360.0, 1.5, 5.0],
    npts=100,
    nstp=3000,
    njmp=20,
    mode="kp",
    ...
    for j in range(nstp):  # loop for iterations
        ...
        jumps = (smax - smin) / 200.0  # step rescaling parameters
        jumpa = (amax - amin) / 200.0
        jumpc = (cmax - cmin) / 200.0
        ...
        while l_iter < njmp:  # random walk to generate a new active point
            ...
            if l_iter >= njmp and naccept == 0:  # start somewhere else
                l_iter = 0
                choose = np.random.randint(0, npts)  # pick a random index
...
def loglikelihood(kpo, index, sep, angle, contrast, mode="kp"):
    ...
    if mode == "kp":
        modl_ker = kpo.kpd_binary_model([sep, angle, contrast], index, "KERNEL")[0]
        chisquared = np.sum(((modl_ker - kpo.__kpd__) / (kpo.__kpe__)) ** 2)

    elif mode == "vis":
        test = kpo.kpd_binary_model([sep, angle, contrast], index, "AMPLI")[0] ** 2
...
def bin_chi2_crit(ker0, kpi, sep, pang, cont, wl, ker_err=None):
    ...
    cvis = core.cvis_binary(kpi.UVC[:, 0], kpi.UVC[:, 1], wl, p1)
    ...
    for kk, sep1 in enumerate(seps):
        for ii, pang1 in enumerate(pangs):
            for jj, con1 in enumerate(cons):
                ...
                sys.stdout.write(
                    "\rsep = %6.2f mas, PA = %5.1f deg, con=%.2f" % (sep1, pang1, con1)
                )
                sys.stdout.flush()
    # ---------------------------------------------
